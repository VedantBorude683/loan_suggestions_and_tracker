...
import {
  LayoutDashboard, Wallet, CreditCard, PieChart as PieChartIcon, ArrowUpRight, ArrowDownRight, Bell, FileText, Download, Calendar, Zap, AlertCircle
} from 'lucide-react';
  ...
  const fetchBudget = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setBudgetAllocations(data.expenses || []);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const categorySpending = React.useMemo(() => {
    return budgetAllocations.map(alloc => {
      const spent = transactions
        .filter(t => t.type === 'expense' && (t.category === alloc.category || (!t.category && t.name === alloc.category)))
        .reduce((acc, curr) => acc + curr.amount, 0);
      return {
        category: alloc.category,
        budgeted: Number(alloc.amount),
        spent: spent,
        remaining: Number(alloc.amount) - spent,
        percent: Number(alloc.amount) > 0 ? (spent / Number(alloc.amount)) * 100 : 0
      };
    });
  }, [budgetAllocations, transactions]);
          ...
          {activeView === "overview" && (
            <>
              ...
              <motion.div className="content-grid" variants={containerVariants} initial="hidden" animate="visible">
                ...
                <motion.div className="chart-section glass-card" variants={itemVariants} style={{ gridColumn: '1 / -1' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    <h3 style={{ margin: 0 }}>Budget Utilization</h3>
                  </div>
                  {categorySpending.length === 0 ? (
                    <div className="empty-state">
                      <p>No budget allocations set for this month. <a href="#" onClick={(e) => { e.preventDefault(); setActiveView('budget'); }} style={{ color: '#6366f1' }}>Set up now</a></p>
                    </div>
                  ) : (
                    <div className="budget-breakdown-grid">
                      {categorySpending.map((cat, idx) => {
                        ...
                        const fillColor = isOver ? '#f43f5e' : (isWarning ? '#f59e0b' : '#38bdf8');
                        const bgGlow = isOver ? 'rgba(244,63,94,0.1)' : (isWarning ? 'rgba(245,158,11,0.1)' : 'rgba(56,189,248,0.1)');
                        const iconColor = isOver ? '#f43f5e' : (isWarning ? '#f59e0b' : '#38bdf8');

                        return (
                          <div key={idx} className="budget-cat-card">
                            <div className="budget-cat-header">
                              <div className="budget-cat-title">
                                <div className="budget-cat-icon" style={{ background: bgGlow, color: iconColor }}>
                                  <PieChartIcon size={18} />
                                </div>
                                {cat.category}
                              </div>
                              <div style={{ fontSize: '1.1rem', fontWeight: 'bold', color: isOver ? '#f43f5e' : '#f8fafc' }}>
                                {cat.percent.toFixed(0)}%
                              </div>
                            </div>

                            <div className="budget-cat-stats">
                              <span style={{ color: '#94a3b8' }}>Spent: <strong style={{ color: isOver ? '#f43f5e' : '#f1f5f9' }}>₹{cat.spent.toLocaleString()}</strong></span>
                              <span style={{ color: '#64748b' }}>of ₹{cat.budgeted.toLocaleString()}</span>
                            </div>
                            ...
                            {isOver && (
                              <div className="budget-cat-alert">
                                <AlertCircle size={14} /> Over budget by ₹{(cat.spent - cat.budgeted).toLocaleString()}
                              </div>
                            )}
                            {!isOver && (
                              <div style={{ fontSize: '0.75rem', color: '#10b981', marginTop: '0.75rem', fontWeight: '500' }}>
